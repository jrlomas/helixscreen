...
| `check_variant_parity.py` | Fail if a `ui_xml/<variant>/` override drops or invents wiring vs its base - widget names, subject bindings, event callbacks, `<api>` props, `cond=`/`*_cond=` presence (#1203). Prop defaults and condition expressions are not compared - they are reflow. Per-file exemptions in `ALLOWED_OMISSIONS`, each with a layout reason |
