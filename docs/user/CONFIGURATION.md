...
expanded or run: a line whose value holds `$(...)`, `${...}` or a backtick is ignored, so
write the final value itself. Only the settings on this page and in the file's own comments
