...
| Tool changer (Z-Mod) | `CHANGE_ZCOLOR SLOT={n} HEX={RRGGBB} TYPE={material} SILENT=1` | `normalize_material()` maps into `zmod_color.valid_types` by compat group, then `is_safe_material_param()` + `gcode_param_value()` (`src/printer/ams_backend_toolchanger.cpp#apply_user_edit`). `TYPE` is required for a prompt-free write, so the send always carries the slot's current colour too, snapped to the firmware palette first. See [FILAMENT_BACKEND_TOOLCHANGER.md](FILAMENT_BACKEND_TOOLCHANGER.md#z-mod-on-the-creator-5-pro) |
...
The strategy is guarded in `open_remap_modal()` (`include/ui_panel_print_select.h`), which asks `PrintPreparationManager::check_modification_capability()` rather than testing for the plugin itself: rewriting a file before printing is the same act the pre-print options perform, and two copies of "may we modify this file" is how one of them ends up offering a picker the other would refuse. `tests/unit/test_gcode_tool_remapper.cpp` pins the three families and the swap collision-safety.
