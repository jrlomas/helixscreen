...
The **print-status panel** path ([`src/ui/ui_panel_print_status.cpp#load_gcode_for_viewing`](../../../src/ui/ui_panel_print_status.cpp#L3554)) checks the gates first — Thumbnail Only mode skips all downloading (`src/ui/ui_panel_print_status.cpp#"skip all gcode downloading/parsing"`), as does `gcode_3d_enabled=false` (`src/ui/ui_panel_print_status.cpp#"bool gcode_3d_enabled = cfg->get<bool>"`) — then probes a persistent cache copy under `get_helix_cache_dir("gcode_temp")` hashed from the filename, and otherwise fetches metadata, applies the shared OOM gate `is_gcode_2d_streaming_safe(size)` (`src/ui/ui_panel_print_status.cpp#"is_gcode_2d_streaming_safe(size)"`), and streams the file to disk via `transfers().download_file_to_path()` (`src/ui/ui_panel_print_status.cpp#"api_->transfers().download_file_to_path("`).

The **detail view** has one canonical entry point, `ensure_gcode_downloaded()` ([`src/ui/ui_print_select_detail_view.cpp#ensure_gcode_downloaded`](../../../src/ui/ui_print_select_detail_view.cpp#L619)), because two consumers need the file — the viewer preview and the tools scan — and they must not race each other into two transfers. Its order matters:

1. **In-place local read.** When Moonraker runs on this host (`is_moonraker_on_same_host()`, `src/system/host_identity.cpp#is_moonraker_on_same_host`), the detail view resolves the `gcodes` root once via `server.files.roots` and reads Moonraker's own copy directly (`local_gcode_source()`, `src/ui/ui_print_select_detail_view.cpp#local_gcode_source`) — no transfer, no second copy on the same flash, guarded by a size match so a stale or partial local file falls back to HTTP. The path is deliberately never registered as a temp download: it is the user's print file, and `reclaim_download()` refuses to delete it.
...
Every deletion of a downloaded copy goes through the single gate `is_reclaimable_download()` (`include/gcode_temp_reclaim.h#is_reclaimable_download`) — only files we created under `gcode_temp` are reclaimable, so a mis-routed path can never delete a print file.

### The footer fast path: tools-used without a download

Before anything downloads, `start_tail_summary_scan()` ([`src/ui/ui_print_select_detail_view.cpp#start_tail_summary_scan`](../../../src/ui/ui_print_select_detail_view.cpp#L1748)) asks one suffix-range question: slicers write `filament used [g]` and the color palette *after the last move*, so the last few tens of KB answer "which tools does this file use, in what colors, at what weight?" with no whole-file download and no geometry parse. When Moonraker runs on this host the same window is read straight off its own copy (`read_file_tail()`, [`include/gcode_footer_summary.h#helix::gcode`](../../../include/gcode_footer_summary.h#L145)) rather than asked for back over loopback. The window is `gcode_tail_window_bytes()` ([`include/gcode_footer_summary.h#helix::gcode`](../../../include/gcode_footer_summary.h#L127)) — exactly `size - gcode_end_byte` when Moonraker's metadata reported the footer offset, clamped to `[16KB, 512KB]`, defaulting to 64KB when it didn't.

The parse itself, `parse_gcode_footer_summary()` ([`include/gcode_footer_summary.h#helix::gcode`](../../../include/gcode_footer_summary.h#L96)), is a pure function run on the HTTP thread (no `this`): exact-key matching so `total filament used [g]` and `default_filament_colour` can't hijack the answer, and `usable()` requires at least one nonzero tool — an all-zero vector is slicer placeholders, not a print that extrudes nothing. What a failure falls through to depends on *what the read was for*, which `footer_read_need()` ([`include/gcode_footer_summary.h#footer_read_need`](../../../include/gcode_footer_summary.h#L191)) decides up front. The footer answers three independent questions — tools, palette, grams — and `ToolsUsedCache` persists only the first, so a cache hit settles `tools` and leaves the other two exactly as unanswered as on a cold open. Only an outstanding **tools** question justifies falling through to the whole-file scan; a read issued for the palette or the grams gives up quietly instead, because reading a hundred-plus megabytes to recover a palette the render can live without is the cost streaming exists to avoid. The deferred apply also discards results that land after the selection moved to a different file.

### The tools-used cache: instant chips on re-open

`ToolsUsedCache` ([`include/tools_used_cache.h#ToolsUsedCache`](../../../include/tools_used_cache.h#L20)) persists the answer as JSON at `get_helix_cache_dir("tools_used")/cache.json`, keyed by `(path, size, mtime)` so a re-sliced file invalidates naturally, LRU-bounded to 256 entries. Three parties touch it:

- `show()` seeds from a cache hit ([`src/ui/ui_print_select_detail_view.cpp#show`](../../../src/ui/ui_print_select_detail_view.cpp#L463)) — the used-tool set is known synchronously and the headless *tools* scan is skipped, so a re-opened file can paint final chips in one frame instead of the skeleton latch. What a hit must **not** do is retire the footer read: the cache says nothing about colors or grams. Gating the whole read on it meant a re-open of a file whose Moonraker metadata carries no `filament_colors` — which is every OrcaSlicer file on a Creality K2 — never recovered a palette, and every tool rendered the neutral stand-in as though the slicer had chosen it (a grey dot pointing at the lane's real color).
...
- The scan-finish path writes through, but **only when authoritative** (`src/ui/ui_print_select_detail_view.cpp#apply_scan_result/"if (authoritative) {"`): a degraded finish (download failed) carries an empty set with no information, and persisting it would freeze "no tools" for the file — on 2D-only platforms no viewer parse ever repairs that.
- The viewer parse writes through when it produces a set (`src/ui/ui_print_select_detail_view.cpp#try_extract_gcode_colors/"if (set_is_authoritative) {"`).

The fallback full scan is itself cheap: a Tn-only line scan on the slow HTTP lane that never holds the whole file and **early-exits once every slicer-palette tool has been seen** ([`include/ui_print_select_detail_view.h#PrintSelectDetailView`](../../../include/ui_print_select_detail_view.h#L873)) — once the palette is covered there is nothing left to learn.
...
The settings surface offers four values ([`include/display_settings_manager.h#DisplaySettingsManager`](../../../include/display_settings_manager.h#L201)): 0=Auto, 1=3D, 2=2D, 3=Thumbnail Only. The widget's enum is the three renderers ([`include/ui_gcode_viewer.h#GcodeViewerRenderMode`](../../../include/ui_gcode_viewer.h#L35)) — Auto resolves at draw time to 3D when the build has the GLES renderer (`ENABLE_GLES_3D`), else 2D ([`src/ui/ui_gcode_viewer.cpp#is_using_2d_mode`](../../../src/ui/ui_gcode_viewer.cpp#L397)); Thumbnail Only is a panel-level setting that skips viewer usage entirely.

The precedence chain is **cmdline > env > settings**, applied in the panel's `on_activate` ([`src/ui/ui_panel_print_status.cpp#on_activate`](../../../src/ui/ui_panel_print_status.cpp#L1146)): `--render-2d`/`--render-3d` (stored in `RuntimeConfig::gcode_render_mode`) win; otherwise `HELIX_GCODE_MODE` — resolved at widget creation by the pure `decide_render_mode()` ([`include/gcode_render_mode_policy.h#decide_render_mode`](../../../include/gcode_render_mode_policy.h#L44)) — stands; otherwise the saved setting is applied. Two traps are load-bearing:
...
All object interaction hit-tests through `GCodeLayerRenderer::pick_object_at()` ([`src/rendering/gcode_layer_renderer.cpp#pick_object_at`](../../../src/rendering/gcode_layer_renderer.cpp#L1585)), a two-stage algorithm:

- **Stage 1 — projected footprints.** Each object's 3D bounding box (accumulated over its whole toolpath) is clamped to the drawn Z range — `render()` only draws up to `current_layer_`, and what is not visible must not be pickable — and its 8 corners projected to a screen-space rect, inflated by `kPickThresholdPx` (15px, `include/gcode_selection_style.h#kPickThresholdPx` and `src/rendering/gcode_layer_renderer.cpp#pick_object_at/"click_pos.x < min_sx - selection::kPickThresholdPx"`). Objects that haven't started printing are skipped, as are supports when supports are hidden (the fast path below never reaches the segment filter that would normally catch them). Zero candidates returns immediately; **a single candidate returns without touching a single segment** — the common case, since objects sit separated on the plate.
- **Stage 2 — segments, downward.** Only when footprints overlap: walk layers from `current_layer_` **downward**, first layer that produces a hit wins (`src/rendering/gcode_layer_renderer.cpp#pick_object_at/"for (int layer_idx = current_layer_; layer_idx >= 0; --layer_idx) {"`) — a tap over a stack picks what is visually on top of it. In streaming mode this walk is **cache-only**: `try_get_layer_segments()` (`src/rendering/gcode_layer_renderer.cpp#pick_object_at/"try_get_layer_segments"`) skips uncached layers rather than seeking and parsing, because a hit-test must never freeze the UI on a 2-core board.

The gestures on top of it ([`src/ui/ui_gcode_viewer.cpp`](../../../src/ui/ui_gcode_viewer.cpp)): a **press-and-hold of 1000ms** (`LONG_PRESS_THRESHOLD_MS`, `src/ui/ui_gcode_viewer.cpp#LONG_PRESS_THRESHOLD_MS` — deliberately double the app-wide 500ms gesture timeout, because the consequence is canceling an object's print) picks the object under the initial press and fires the long-press callback into the exclusion flow; a **tap** (release with minimal movement, no pinch, long-press not already fired) toggles single-selection (`src/ui/ui_gcode_viewer.cpp#gcode_viewer_release_cb`), which draws the corner-bracket wireframe via `render_selection_brackets()` ([`src/rendering/gcode_layer_renderer.cpp#render_selection_brackets`](../../../src/rendering/gcode_layer_renderer.cpp#L1822)). The **skip-objects button** (`btn_objects`, [`ui_xml/components/print_status_preview_card.xml#btn_objects`](../../../ui_xml/components/print_status_preview_card.xml#L38), visible when the `exclude_objects_available` subject is set) toggles the map + side-list panel (`on_objects_clicked`, [`src/ui/ui_panel_print_status.cpp#on_objects_clicked`](../../../src/ui/ui_panel_print_status.cpp#L2107)).
...
- **Streaming and full-file are mutually exclusive by construction** — each setter nulls the other's pointer ([`src/rendering/gcode_layer_renderer.cpp#set_gcode`](../../../src/rendering/gcode_layer_renderer.cpp#L117) vs `src/rendering/gcode_layer_renderer.cpp#set_streaming_controller`). Never hand a renderer both; the second call silently wins.
...
1. [`include/gcode_parser.h#ToolpathSegment`](../../../include/gcode_parser.h#L222) — `ToolpathSegment`, `Layer`, `GCodeObject`, then `ParsedGCodeFile` at `include/gcode_parser.h#ParsedGCodeFile`: the interned object-name design (`object_name_table`) that picking compares with `int16_t`s.
2. [`src/ui/ui_gcode_viewer.cpp#ui_gcode_viewer_load_file_async`](../../../src/ui/ui_gcode_viewer.cpp#L1521) — Phase 0: the streaming decision in five lines, then the two paths diverging at [`src/ui/ui_gcode_viewer.cpp#ui_gcode_viewer_load_file_async`](../../../src/ui/ui_gcode_viewer.cpp#L1590) (streaming) and [`src/ui/ui_gcode_viewer.cpp#ui_gcode_viewer_load_file_async`](../../../src/ui/ui_gcode_viewer.cpp#L1745) (full-load).
3. [`include/gcode_streaming_config.h#helix`](../../../include/gcode_streaming_config.h#L11) — the three-step config hierarchy; then `should_use_gcode_streaming()` at `include/gcode_streaming_config.h#"bool should_use_gcode_streaming(size_t file_size_bytes);"`.
4. [`include/gcode_streaming_controller.h#"class GCodeStreamingController {"`](../../../include/gcode_streaming_controller.h#L169) — the controller: cache budget floor, prefetch radius, and `try_get_layer_segments()`'s shared_ptr contract at `include/gcode_streaming_controller.h#try_get_layer_segments`.
5. [`src/ui/ui_print_select_detail_view.cpp#ensure_gcode_downloaded`](../../../src/ui/ui_print_select_detail_view.cpp#L619) — `ensure_gcode_downloaded()`: the local-read/join/disk/start order and why each precedes the next.
6. [`src/ui/ui_print_select_detail_view.cpp#local_gcode_source`](../../../src/ui/ui_print_select_detail_view.cpp#L574) — `local_gcode_source()`: the same-host probe and size staleness check; then `reclaim_download()` at `src/ui/ui_print_select_detail_view.cpp#reclaim_download` for the delete gate.
7. [`include/gcode_footer_summary.h#GcodeFooterSummary`](../../../include/gcode_footer_summary.h#L23) — the whole footer contract in one header: `GcodeFooterSummary`, the exact-key rules at `include/gcode_footer_summary.h#"Recognizes exactly two keys"`, and the window bounds at `include/gcode_footer_summary.h#GCODE_TAIL_WINDOW_MIN`.
8. [`src/ui/ui_print_select_detail_view.cpp#start_tail_summary_scan`](../../../src/ui/ui_print_select_detail_view.cpp#L1748) — `start_tail_summary_scan()`: the local-vs-HTTP tail read, pure parse off the main thread, stale-file guard, and the need-dependent fall-through.
9. [`include/tools_used_cache.h#ToolsUsedCache`](../../../include/tools_used_cache.h#L20) — the cache: keying, LRU bound, and the "empty set is legitimate" contract at `include/tools_used_cache.h#"An EMPTY set is a legitimate"`. Then its three call sites in the detail view ([`src/ui/ui_print_select_detail_view.cpp#show`](../../../src/ui/ui_print_select_detail_view.cpp#L463), [`src/ui/ui_print_select_detail_view.cpp#try_extract_gcode_colors`](../../../src/ui/ui_print_select_detail_view.cpp#L1258), [`src/ui/ui_print_select_detail_view.cpp#apply_scan_result`](../../../src/ui/ui_print_select_detail_view.cpp#L1647)).
10. [`include/gcode_render_mode_policy.h#decide_render_mode`](../../../include/gcode_render_mode_policy.h#L44) — `decide_render_mode()`: case-sensitive matching and the unrecognized-is-2D asymmetry, in 60 lines.
11. [`src/ui/ui_panel_print_status.cpp#on_activate`](../../../src/ui/ui_panel_print_status.cpp#L1146) — the precedence chain applied; then the pinned-mode guard at `src/ui/ui_panel_print_status.cpp#PrintStatusPanel/"command line pinned"`.
12. [`src/rendering/gcode_layer_renderer.cpp#pick_object_at`](../../../src/rendering/gcode_layer_renderer.cpp#L1585) — `pick_object_at()` in full: stage-1 clamping and skips starting at `src/rendering/gcode_layer_renderer.cpp#pick_object_at/"Stage 1: candidate objects by projected bounding box."`, the single-candidate return (`src/rendering/gcode_layer_renderer.cpp#pick_object_at/"candidate_count == 1"`), the downward walk (`src/rendering/gcode_layer_renderer.cpp#pick_object_at/"for (int layer_idx = current_layer_; layer_idx >= 0; --layer_idx) {"`), and the cache-only streaming branch (`src/rendering/gcode_layer_renderer.cpp#pick_object_at/"try_get_layer_segments"`).
13. [`src/ui/ui_gcode_viewer.cpp#gcode_viewer_draw_cb`](../../../src/ui/ui_gcode_viewer.cpp#L882) — the long-press constant and its comment; then the release callback's tap handling at `src/ui/ui_gcode_viewer.cpp#gcode_viewer_release_cb`.
14. [`ui_xml/components/print_status_preview_card.xml#btn_objects`](../../../ui_xml/components/print_status_preview_card.xml#L38) — `btn_objects`: the `exclude_objects_available` binding and the `on_print_status_objects` callback — the whole skip-objects entry point in eight lines of XML.
