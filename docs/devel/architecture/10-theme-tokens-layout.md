...
| `theme_manager_get_color()` call sites | 433 | `rg -c 'theme_manager_get_color\(' src include \| awk -F: '{s+=$2} END {print s}'` |
...
- **One lookup rule for C++:** `theme_manager_get_color("card_bg")` for tokens — it tries `{base}_light`/`{base}_dark` and falls back to the bare name ([`src/ui/theme_manager.cpp`](../../../src/ui/theme_manager.cpp)). `theme_manager_parse_hex_color()` parses a hex *literal* only; feeding it a token name returns black. 433 call sites in `src/` + `include/` follow this split, and two ratcheting gates keep it honest: [`scripts/quality-checks.sh`](../../../scripts/quality-checks.sh) fails on `lv_color_hex(0x…)` above a baseline (33, exemptions for theme_manager itself and procedural renderers), and [`scripts/check_hardcoded_pixels.py`](../../../scripts/check_hardcoded_pixels.py) caps raw pixel literals in XML (150).
