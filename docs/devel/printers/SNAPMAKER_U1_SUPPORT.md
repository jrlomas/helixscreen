...
AI-based print defect detection system. The status object itself is not subscribed — HelixScreen consumes detection through `print_stats.exception` instead: on a confirmed defect (custom `exception` sub-object, `code: 2` = spaghetti/noodle) the firmware pauses the print itself, and `U1StockSource` reports it to `DetectionManager` (`include/detection_manager.h`) with `already_paused` set. Sources only report: `DetectionManager::response_for()` classifies the event from the two settings in Settings > Safety & Notifications (**Spaghetti Detection**, **Pause on Detection** — exposed to XML as the `detection_available` subject), and the presenter pauses only when a pause is wanted and the printer has not already done it, so a U1 is never double-paused. Pause-on-detect off means a warn-only toast. The U1 firmware keeps no preference HelixScreen can read, so the first capable start seeds the settings with the defaults (detection on, pause on) and they are HelixScreen's from then on. Capability is gated by probing `printer.objects.list` for `defect_detection` after the WebSocket connects (probing at init would latch false forever).
