...
| Timeouts wait for both heaters at target (2°C) and count a climbing heater as activity (a new high under the current target, not a swing back up); the ceiling ignores temps and heaters but waits out narration; the backstop ignores everything | `test_print_start_collector.cpp` - "Timeout fallback waits for the heaters to reach their targets", "A heater still closing on its target counts as activity", "A heater cycling under its target is not climbing", "The ceiling ends a pre-print whose heater never settles", "The ceiling waits for a narrating printer to go quiet", "A printer that never stops narrating ends at the backstop", "The ceiling stretches for a long prediction" |
| A pattern's declared hold (a COSMOS heat soak's silent G4) counts as activity until it ends and is left out of the ceiling | `test_print_start_collector.cpp` - "a long COSMOS heat soak holds the pre-print open"; `test_print_start_profile_cosmos.cpp` - "A heat soak holds the pre-print for the minutes it announces" |
| A HEATING_BED or BED_MESH pattern relabels its phase in place; no pattern pulls a later phase back | `test_print_start_collector.cpp` - "A heat soak line relabels a bed that is already heating" |
| An idle mesh gives way to the nozzle heating toward a target set after the mesh began | `test_print_start_collector.cpp` - "a quiet COSMOS park and purge shows the nozzle heating" |
